...
public class Matrix {
	...
	public Matrix(int d[][])
	{
		...
		for(int rowCount = 0; rowCount< numRows; rowCount++) {
			for (int colCount = 0; colCount < numColumns; colCount++ ) {
				data[rowCount][colCount] = d[rowCount][colCount] ;
			}
		}
	...
	public String toString()
	{
		...
		for(int rowCount = 0; rowCount < numRows; rowCount++) {
			for(int colCount = 0; colCount < numColumns; colCount++) {
			int numBeingTransferred = data[rowCount][colCount]; 
			mtrxHolder += numBeingTransferred + " ";
			}
		mtrxHolder += "\n";
		}
	...
	public boolean equals(Object o)
	{
		...
		if((m.data.length != data.length) || (m.data[0].length != data[0].length)) {
			...
			for(int rowCount = 0; rowCount < numRows; rowCount++) {
				for(int colCount = 0; colCount < numColumns; colCount++) {
					if (m.data[rowCount][colCount] != data[rowCount][colCount]) {
						return false; 
					}
	...
	public Matrix mult(Matrix m)
	{
		...
		int columns = numRows; 
		int rows = m.numColumns;
		Matrix endAnswer = new Matrix(columns,rows);
		for (int row = 0; row < numRows; row++) {
			for (int col = 0; col < m.numColumns; col++) {
				for (int num = 0; num < numColumns; num++) {
					endAnswer.data[row][col] += data[row][num] * m.data[num][col]; 
				}
	...
	public Matrix add(Matrix m)
	{
		...
		Matrix end = new Matrix(m.numRows, m.numColumns); 
		for(int rowCount = 0; rowCount< numRows; rowCount++) {
			for (int colCount = 0; colCount < numColumns; colCount++ ) {
				end.data[rowCount][colCount] = data[rowCount][colCount] + m.data[rowCount][colCount];
			}
    ...
    {
		Matrix transposed = new Matrix(data[0].length,data.length );
		  for(int rowCount = 0; rowCount < data[0].length ; rowCount++) {
			  for(int colCount = 0; colCount < data.length; colCount++) {
				  transposed.data[rowCount][colCount] = data[colCount][rowCount];
